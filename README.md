...
BasicPad is a lightweight desktop text editor built with Python and Tkinter.
...
- `basicpad.py`: Tkinter editor application entry point
- `basicpad.spec`: PyInstaller build configuration
- `assets/`: App icon assets used by the runtime window and packaging
- `packaging/basicpad.desktop`: Desktop entry used for Linux packaging
- `build-appimage.sh`: Builds a Linux AppImage from the PyInstaller binary
- `run-basicpad.sh`: Launches the app from source
- `start-basicpad.sh`: Launches the packaged binary from `dist/basicpad`
...
- Linux: `~/.config/basicpad/config.json`
- Cache: `~/.cache/basicpad/tts/`
...
PyInstaller spec is provided as `basicpad.spec`.
...
- `assets/basicpad-icon.ppm` is used by the Tkinter window at runtime
- `assets/basicpad-icon.svg` is used for Linux desktop/AppImage packaging
- `packaging/basicpad.desktop` defines the launcher metadata and icon name
...
- `dist/basicpad` from PyInstaller
- `BasicPad-x86_64.AppImage` from `appimagetool`
...
- If you later want native distro packages, the same `basicpad.desktop` and icon assets can be reused for `.deb` or `.rpm` packaging.

## Development Roadmap

1. Introduce modular package layout (`basicpad/` package, not only single-file script).
...
- Refactor `basicpad.py` into modules (`ui`, `core`, `tts`, `audio`, `config`).
